...
<html>
  ...
  require_once("koneksi.php");
	...
	?>
  <center>
  <h1><b>INPUT BOBOT KRITERIA</b></h1>
  ...
  <center>
	...
	$data_baru1=mysqli_query($db_link,$sql1); 	
        while ($data_pegawai=mysqli_fetch_array($data_baru1)){
          echo "<tr bgcolor='#FFFFFF'>";
                 echo "<td>{$data_pegawai['id_bagian']}</td>"
                  ."<td>{$data_pegawai['bagian']}</td>";
				  

          $sql2="SELECT  * FROM bagian B, kriteria_bagian K
		  		WHERE B.id_bagian=K.id_bagian
                  AND B.id_bagian='".$data_pegawai['id_bagian']."'";
          $data_baru2=mysqli_query($db_link,$sql2);
          while($new_kriteria=mysqli_fetch_array($data_baru2)){
            echo "<td align='center'>{$new_kriteria['bobot']}</td>";
          }
          echo "<td><a href='bobot_form.php?id_bagian={$data_pegawai['id_bagian']}'>Input</a></td>";
          echo " </tr>";
